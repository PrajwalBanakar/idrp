<!-- src/views/AdminBoardMembersView.vue -->
...
import {
  ...
  type BoardMemberAdminItem,
  type BoardMemberAdminPayload,
} from '@/services/boardMemberAdminService'

const members = ref<BoardMemberAdminItem[]>([])
...
const form = reactive<BoardMemberAdminPayload>({
  name: '',
  designation: '',
  boardType: '',
  organization: '',
...
const loadMembers = async () => {
  ...
  try {
    members.value = await fetchAdminBoardMembers()
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Failed to load board members'
  } finally {
    loading.value = false
  }
}

const editMember = (member: BoardMemberAdminItem) => {
  editingId.value = member.id

  form.name = member.name
  form.designation = member.designation || ''
  form.boardType = member.boardType || ''
...
const submitMember = async () => {
  ...
  const payload: BoardMemberAdminPayload = {
    name: form.name.trim(),
  ...
  try {
    if (editingId.value) {
      await updateAdminBoardMember(editingId.value, payload)
      successMessage.value = 'Board member updated successfully'
    } else {
      await createAdminBoardMember(payload)
      successMessage.value = 'Board member created successfully'
    }

    resetForm()
    await loadMembers()
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Failed to save board member'
  } finally {
    saving.value = false
  }
}

const removeMember = async (member: BoardMemberAdminItem) => {
  ...
  try {
    await deleteAdminBoardMember(member.id)
    successMessage.value = 'Board member deleted successfully'
    ...
    error.value = err instanceof Error ? err.message : 'Failed to delete board member'
  }
}
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p class="mt-2 text-sm text-slate-600">
          Manage board member profiles, roles, organizations, and display order.
        </p>
      </div>
                ...
                <td class="px-4 py-4">
                  <div class="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p v-if="member.organization" class="mt-1 text-xs text-slate-500">
                        {{ member.organization }}
                      </p>
                    </div>
                  </div>
